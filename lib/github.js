...
var mongo = 'mongodb://jade-bot:' + TOKEN + '@ds059917.mongolab.com:59917/jade-bot'
var db = mongojs(mongo, ['repos'])
...
function processRepo(user, repo, branch, callback) {
  var _id = user + '/' + repo + '/' + branch
  db.repos.find({_id: _id}, function (err, res) {
    if (err) return callback(err)
      ...
      pullRequests(user, repo)
        .on('data', function (pr) {
          if (/\[bot-update#1\]/.test(pr.title)) {
            ...
            db.repos.update({_id: _id}, {$set: { 'state-1': 'pull-request-submitted'}}, {upsert: true}, function (err) {
              callback(err, false)
            })
          }
        })
          ...
          gethub(user, repo, branch, dir, function (err) {
            ...
            if (files.length === 0) {
              ...
              return rimraf(dir, function (err) {
                db.repos.update({_id: _id}, {$set: { 'state-1': 'no-change-needed'}}, {upsert: true}, function (err) {
                  callback(err, false)
                })
              })
            }
